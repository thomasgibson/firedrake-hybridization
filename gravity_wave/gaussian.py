...
__all__ = ["GaussianExpression", "MultipleGaussianExpression"]


class GaussianExpression(object):
    """Expression for Gaussian.

    Defines a Gaussian which is centred in the direction given by the
    vector :math:`\\vec{n}_0` and the radius :math:`r_0`. The width
    (in angular distance from the direction :math:`\\vec{n}_0`) is
    given as :math:`\sigma_{\\theta}` and in the height it is given
    as :math:`\sigma_r`.
    The amplitude is :math:`A`,
    :math: A\cdot exp(-((\\theta-\\theta_0)^2/\sigma_{\\theta}^2+(r-r_0)^2/\sigma_r^2)/2)
...
class MultipleGaussianExpression(object):
    ...
    with variable directions, widths and heights. The directions are
    drawing randomly and are uniformly distributed over the surface of a
    sphere. Same for mean heights, which are uniformly distributed through
    the entire thickness of the atmosphere. The widths both in the angle and
    in height decrease uniformly, with the maximal angular width being 1.0
    and the maximal width in the radial direction being the thickness
    of the atmospher.
