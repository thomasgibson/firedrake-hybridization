...
ref_dt = {3: 1800., 4: 900., 5: 450., 6: 225.}
...
refinements = args.refinements    # Number of refinements
...
##############################################################################
# Diagnostic solve for the potential vorticity
##############################################################################

P3 = FunctionSpace(mesh, "CG", model_degree + 1)
qn = Function(P3, name='pv')
gammar = TestFunction(P3)
qq = TrialFunction(P3)
pv_a = inner(qq, gammar*hn)*dx
gradperp = lambda psi: cross(outward_normals, grad(psi))
pv_L = gammar*f*dx - inner(gradperp(gammar), un)*dx
pvproblem = LinearVariationalProblem(pv_a, pv_L, qn)
pvsolver = LinearVariationalSolver(pvproblem,
                                   solver_parameters={'ksp_type': 'preonly',
                                                      'pc_type': 'lu',
                                                      'pc_factor_mat_solver_type': 'mumps'},
                                   options_prefix='pv_solver')
...
while t < tmax:
    ...
    PETSc.Sys.Print("Time step, t=%s completed." % t)
